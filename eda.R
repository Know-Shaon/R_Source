...
# Create a sample of 50 numbers which are nearly normally distributed.
y <- rnorm(50)  # take random numbers from a normaldistribution 
D = density(y)
plot(D)

#-------
y <- rnorm(50,mean=0,sd=100)
D = density(y)
plot(D)
---------------
...
#Normal distribution 
plot(dnorm(seq(-25,25,.2),mean=0,sd=5))
qnorm(.95,mean=0,sd=1)  # z-score at a specific Percentile 
pnorm(2,mean=0,sd=1)  # Percentile at a specific z-score

#Student distribution 
 qt(.95,df=30)  # z-score at a specific Percentile 
...
dist1=as.data.frame(x)
dist1[,2]=1

x=rnorm(200,mean=5,sd=10)
dist2=as.data.frame(x)
dist2[,2]=2

my_data=rbind(dist1,dist2)
my_data$V2=as.factor(my_data$V2)
ggplot(my_data,aes(x=x,fill=V2))+geom_density()

result <- aov(x~V2,data = my_data)
summary(result)
